<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Tipe Training: ') . $pelatihan->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-xl">
                {{-- PERBAIKAN: $training -> $pelatihan (dan ['pelatihan' => $pelatihan] tidak diperlukan jika nama cocok) --}}
                <form method="POST" action="{{ route('admin.pelatihan.update', $pelatihan) }}">
                    ...
                    <div class="p-6 lg:p-8 text-gray-900 dark:text-gray-100 space-y-6">
                        <div class="mb-4">
                            ...
                            {{-- PERBAIKAN: $training -> $pelatihan --}}
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $pelatihan->name)" required autofocus />
                        ...
                        <div class="block mb-4">
                            <label for="is_active" class="inline-flex items-center">
                                {{-- PERBAIKAN: $training -> $pelatihan --}}
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $pelatihan->is_active) ? 'checked' : '' }} class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm">
